...
const COLORS: Record<S,string> = {
  RECEIVED:"bg-blue-100 text-blue-700", KITCHEN:"bg-amber-100 text-amber-700",
...
export default function OrderStatusSelect({ orderId, current }: { orderId: string; current: string }) {
  ...
  const [value, setValue] = useState<S>(current as S);
  const [saving, setSaving] = useState(false);

  const change = async (next: S) => {
    if (next === value) return;
    setSaving(true);
    const res = await fetch(`${BACKEND}/api/orders/${orderId}`, { method:"PATCH", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ status: next }), credentials:"include" });
    if (res.ok) { setValue(next); router.refresh(); }
    setSaving(false);
  };

  return (
    <select value={value} onChange={e => change(e.target.value as S)} disabled={saving}
